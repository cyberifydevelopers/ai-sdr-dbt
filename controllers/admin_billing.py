...
# ── VAPI BASE RATE (ENV OVERRIDABLE) ─────────────────────────────────────────
# Default: $0.05 per minute. Override with VAPI_RATE_PER_MIN_DOLLARS="0.05"
_raw_vapi_rate = os.getenv("VAPI_RATE_PER_MIN_DOLLARS", "0.05").strip()
try:
    VAPI_RATE_PER_MIN_DOLLARS = Decimal(_raw_vapi_rate)
except (InvalidOperation, TypeError):
    VAPI_RATE_PER_MIN_DOLLARS = Decimal("0.05")
...
async def get_pricing_admin(user: Annotated[User, Depends(get_current_user)]):
    ...
    call_cents_per_second = Decimal(call_mps) / Decimal(1000)
    call_dollars_per_second = (call_cents_per_second / Decimal(100))
    text_dollars_per_message = Decimal(text_cents) / Decimal(100)
...
# ── NEW: VAPI BASE PRICE (ADMIN VIEW) ────────────────────────────────────────
@router.get("/billing/vapi/actual-cost")
async def vapi_actual_cost(
    user: User = Depends(get_current_user),
):
    """
    Admin-only: Vapi base platform charge snapshot (env-overridable).

    Defaults to $0.05 per minute. Returns:
      - per_minute_dollars
      - per_second_dollars (rounded to 6 d.p.)
      - per_second_cents   (rounded to 6 d.p.)
      - per_second_millicents (float), and integer-rounded millicents for convenience
    """
    _ensure_admin(user)

    per_min_dollars = VAPI_RATE_PER_MIN_DOLLARS  # Decimal
    per_sec_dollars = (per_min_dollars / Decimal(60)).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
    per_sec_cents = (per_sec_dollars * Decimal(100)).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
    per_sec_millicents_float = (per_sec_cents * Decimal(1000)).quantize(Decimal("0.000001"), rounding=ROUND_HALF_UP)
    per_sec_millicents_rounded = int(per_sec_millicents_float.quantize(0, rounding=ROUND_HALF_UP))  # integer ≈ 83

    return {
        "provider": "vapi",
        "currency": "USD",
        "per_minute_dollars": float(per_min_dollars),          # 0.05
        "per_second_dollars": float(per_sec_dollars),          # 0.000833
        "per_second_cents": float(per_sec_cents),              # 0.083333
        "per_second_millicents": float(per_sec_millicents_float),  # 83.333
        "per_second_millicents_rounded": per_sec_millicents_rounded,  # 83
        "env": {
            "VAPI_RATE_PER_MIN_DOLLARS": str(per_min_dollars)
        },
        "note": "Base platform charge from Vapi (defaults to $0.05/min). Override via VAPI_RATE_PER_MIN_DOLLARS."
    }
