...
#////////////////////////////////////  Delete Assistant (UPDATED with robust logging & fallbacks) /////////////////////////////////////////
@router.delete("/assistants/{assistant_id}")
async def delete_assistant(assistant_id: int, user: Annotated[User, Depends(get_current_user)]):
    print(f"[DELETE_ASSISTANT] Start | assistant_id={assistant_id} | user_id={getattr(user, 'id', None)}")
    vapi_meta = {"attempted": False, "url": None, "status": None, "body": None, "error": None}
    phone_meta = {"detached_on_vapi": False, "error": None}

    try:
        assistant = await Assistant.get_or_none(id=assistant_id)
        if not assistant:
            print(f"[DELETE_ASSISTANT] Not found id={assistant_id}")
            raise HTTPException(status_code=404, detail="Assistant not found")

        # 1) Detach phone number on VAPI if present (best-effort)
        if assistant.vapi_phone_uuid:
            try:
                phone_url = f"https://api.vapi.ai/phone-number/{assistant.vapi_phone_uuid}"
                print(f"[DELETE_ASSISTANT] PATCH phone detach -> {phone_url}")
                resp = requests.patch(
                    phone_url,
                    json={"assistantId": None},
                    headers=get_headers(),
                ...
                phone_meta["detached_on_vapi"] = resp.ok
                try:
                    print(f"[DELETE_ASSISTANT] phone detach status={resp.status_code} body={resp.text}")
                except Exception:
                    pass
            except requests.RequestException as e:
                phone_meta["error"] = repr(e)
                print(f"[DELETE_ASSISTANT] WARN phone detach failed: {repr(e)}")
                print(traceback.format_exc())

        # 2) Clear local phone_number linkage (non-fatal if not found)
        try:
            if assistant.attached_Number:
                phone_number = await PurchasedNumber.get_or_none(attached_assistant=assistant_id)
                if phone_number:
                    print(f"[DELETE_ASSISTANT] Clearing local purchased_number.attached_assistant for {phone_number.phone_number}")
                    await PurchasedNumber.filter(attached_assistant=assistant_id).update(attached_assistant=None)
        except Exception as e:
            print(f"[DELETE_ASSISTANT] WARN clearing local phone linkage failed: {repr(e)}")
            print(traceback.format_exc())

        # 3) Delete on VAPI (best-effort; tolerate 404/204/400)
        vapi_assistant_id = assistant.vapi_assistant_id
        if vapi_assistant_id:
            vapi_base = os.getenv("VAPI_URL", "https://api.vapi.ai/assistant")
            vapi_url = f"{vapi_base.rstrip('/')}/{vapi_assistant_id}"
            vapi_meta["attempted"] = True
            vapi_meta["url"] = vapi_url
            print(f"[DELETE_ASSISTANT] DELETE VAPI -> {vapi_url}")

            try:
                response = requests.delete(vapi_url, headers=get_headers(), timeout=30)
                vapi_meta["status"] = response.status_code
                try:
                    vapi_meta["body"] = response.text
                except Exception:
                    vapi_meta["body"] = None

                print(f"[DELETE_ASSISTANT] VAPI delete status={response.status_code}")
                try:
                    print(f"[DELETE_ASSISTANT] VAPI delete body={response.text}")
                except Exception:
                    pass

                # Treat these as success: 200, 201, 204, 404 (already gone), and even 400 (invalid id) to avoid blocking local delete
                if response.status_code not in (200, 201, 204, 404, 400):
                    # Log but don't stop local deletion
                    print(f"[DELETE_ASSISTANT] WARN VAPI delete unexpected status {response.status_code}")
            except requests.RequestException as e:
                vapi_meta["error"] = repr(e)
                print(f"[DELETE_ASSISTANT] WARN VAPI delete failed: {repr(e)}")
                print(traceback.format_exc())
        else:
            print("[DELETE_ASSISTANT] No vapi_assistant_id on record; skipping VAPI delete")

        # 4) Finally, delete the local assistant (authoritative)
        await assistant.delete()
        print(f"[DELETE_ASSISTANT] SUCCESS local delete | assistant_id={assistant_id}")

        # Keep original response shape for your UI, but include meta for debugging
        return {
            "success": True,
            "detail": "Assistant has been deleted.",
            "vapi": vapi_meta,
        ...
        print(f"[DELETE_ASSISTANT] HTTPException {http_exc.status_code} | {http_exc.detail}")
        raise http_exc
    except Exception as e:
        print(f"[DELETE_ASSISTANT] UNEXPECTED EXCEPTION: {repr(e)}")
        print(traceback.format_exc())
        # Even on unexpected error, surface details clearly
        raise HTTPException(
            status_code=500,
            detail=f"Server error during delete: {type(e).__name__}: {str(e)} | Traceback: {traceback.format_exc()}"
        )
