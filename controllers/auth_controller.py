...
#             await code.delete()
#             token = generate_user_token({ "id": user.id })
#             print(f"Token at verification: {token}")
#             return {
#                 "success": True,
#                 "token": token,
#                 "user": {
...
#         hashed_new_password = ph.hash(payload.new_password)
...
#     """
#     Build absolute URL from a path like '/media/profile_photos/abc.jpg'
#     """
...
# # >>> NEW: Upload/Update Profile Photo
# #////////////////////////////////////  Upload/Update Profile Photo  /////////////////////////////////////////////////
# @auth_router.post("/profile-photo")
# async def upload_or_update_profile_photo(
#     request: Request,
...
#     - Max size: 5MB
#     - Replaces previous photo and deletes old file if present
...
#         raise HTTPException(
#             status_code=400,
#             detail=f"Image too large. Max allowed is {MAX_PROFILE_PHOTO_BYTES // (1024*1024)} MB.",
#         )

#     ext = _ext_from_content_type(file.content_type)
#     if not ext:
#         raise HTTPException(status_code=400, detail="Could not determine file extension from content type.")
...
#     # Delete previous file if any (best-effort)
#     old_rel = getattr(user, "profile_photo", None)
#     if old_rel:
#         try:
#             candidate = Path(".") / old_rel.lstrip("/")
#             if not candidate.exists():
#                 candidate = Path(PROFILE_PHOTO_STORAGE) / Path(old_rel).name
#             if candidate.exists() and candidate.is_file():
#                 candidate.unlink(missing_ok=True)
#         except Exception:
#             pass

#     # store relative URL in DB; return absolute in response
#     relative_url = f"{PROFILE_PHOTO_URL_PATH}/{new_filename}".replace("//", "/")
...
#         "photo_url": _abs_url(request, relative_url),  # ABSOLUTE
#         "filename": new_filename,
#     }

# # >>> NEW: Get/Delete Profile Photo
# #////////////////////////////////////  Get Profile Photo (URL or redirect)  /////////////////////////////////////////
...
# ):
#     """
#     Returns the absolute URL to the current user's profile photo.
#     If `?redirect=true`, responds with a 302 redirect to the image URL.
#     """
#     rel = getattr(user, "profile_photo", None)
#     if not rel:
#         raise HTTPException(status_code=404, detail="No profile photo set.")

#     abs_url = _abs_url(request, rel)
#     if redirect:
...
# ):
#     """
#     Delete the current user's profile photo from disk and clear the field.
#     """
...
#     # keep relative in DB, but provide absolute URL to the client
#     rel = getattr(user, "profile_photo", None)
...
#             "email": user.email,
#             "role": getattr(user, "role", None),
#             "profile_photo": rel,            # relative (e.g., /media/profile_photos/abc.jpg)
#             "profile_photo_url": abs_url,    # absolute (e.g., https://site.com/media/profile_photos/abc.jpg)
#         }
#     }
