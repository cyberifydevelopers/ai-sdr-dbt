...
#             # NOTE: multiple scopes are COMMA-separated
...
#         elif crm == "zoho":
#             # Use a modules endpoint compatible with ZohoCRM.modules.ALL
...
#             r = await client.get(f"{api_base}/crm/v2/Contacts?fields=id&per_page=1", headers=zheaders)
#             if r.status_code not in (200, 204):
#                 # surface useful info for debugging if needed
#                 raise HTTPException(401, "Zoho token validation failed.")
...
#         acc.expires_at = _exp_from_seconds(int(j.get("expires_in", 3600)))
#     else:
#         acc.access_token = j.get("access_token")
#         acc.refresh_token = acc.refresh_token or j.get("refresh_token")
#         acc.expires_at = _exp_from_seconds(int(j.get("expires_in", 3600)))
...
#             acc.instance_url = j.get("api_domain") or acc.instance_url
#         if crm == "zoho":
#             acc.instance_url = j.get("api_domain") or acc.instance_url or _zoho_api_base_from_token_url(cfg["token_url"])
...
#         elif crm == "zoho":
#             # Determine API and Accounts bases
#             api_base = (acc.instance_url or _zoho_api_base_from_token_url(_cfg()['zoho']['token_url'])).rstrip("/")
...
#             # Fill display name/email if available
#             try:
#                 info = r_acc.json()
...
#             # 2) Try light CRM probes. If they fail (brand-new org/modules), accept but don't raise.
#             endpoints = [
...
#                     ok = True
#                     break
#                 last_status, last_text = r.status_code, (r.text or "")[:300]

#             # If CRM probes all failed, we still consider the token verified (Accounts OK).
#             # Keep the connection and let first real API call show granular error if needed.
#             if not ok:
#                 # Optional: log or attach a note by setting name if empty
#                 acc.external_account_id = acc.external_account_id or ""
#                 # Do NOT raise here to avoid blocking setup for fresh orgs.
...
#             acc.instance_url = j.get("api_domain") or acc.instance_url
#         if crm == "zoho":
#             # Prefer api_domain; else map from accounts-server; else derive from token_url
#             api_base = j.get("api_domain") or _zoho_api_base_from_accounts_server(accounts_server) \
#                        or _zoho_api_base_from_token_url(cfg["token_url"])
#             acc.instance_url = api_base

#     # Verify & save; for Zoho we allow Accounts-verified even if CRM probe fails
#     try:
#         await _verify_and_fill_account(crm, acc)
#     except HTTPException as e:
#         if crm == "zoho":
#             # Accept Accounts-verified flows: retry Accounts userinfo; if OK, proceed anyway.
#             try:
...
#                     if r.status_code in (200, 204):
#                         # proceed without raising, token is valid
#                         pass
#                     else:
#                         await acc.save()
#                         await st.delete()
...
#     # ensure fresh oauth tokens (no-op for Close)
...
#     cursor: Optional[str] = Query(None, description="Provider-specific cursor (HubSpot: after, Pipedrive: start, Salesforce: offset, Zoho: page, Close: skip)"),
#     page: Optional[int] = Query(None, description="Zoho helper: page number (1-based)"),
# ):
#     """
#     Unified contacts listing across CRMs.
#     Output: items[], next_cursor, raw_paging
#     """
#     crm = crm.lower()
#     if crm not in SUPPORTED:
#         raise HTTPException(400, "Unsupported CRM")
...
#                 name = f"{p.get('firstname','')}".strip()
#                 lname = p.get("lastname")
#                 if lname:
#                     name = f"{name} {lname}".strip()
...
#             api_base = (acc.instance_url or _zoho_api_base_from_token_url(_cfg()["zoho"]["token_url"])).rstrip("/")
#             zheaders = {"Authorization": f"Zoho-oauthtoken {acc.access_token}", "Accept": "application/json"}
#             # Zoho paging: page (1-based) + per_page
...
#                 phone = c.get("Phone")
#                 acct = None
#                 if isinstance(c.get("Account_Name"), dict):
#                     acct = c["Account_Name"].get("name")
...
#         elif crm == "salesforce":
#             # Use SOQL; determine latest api version
...
#             # Prefer nextRecordsUrl if available
#             if not j.get("done"):
#                 next_cursor = str(offset + limit)

#         elif crm == "close":
#             # Close CRM Contacts (people)
#             # NOTE: Close has Leads (companies) and Contacts (people)
...
#                 if emails:
#                     email = (emails[0].get("email") if isinstance(emails[0], dict) else emails[0]) or None
#                 if phones:
#                     phone = (phones[0].get("phone") if isinstance(phones[0], dict) else phones[0]) or None
#                 # company via lead_id? (raw me available rahega)
#                 items.append(_norm(c.get("id"), name, email, phone, None, c))
#             raw_paging = {"has_more": j.get("has_more"), "next": j.get("next")}
#             if j.get("has_more"):
#                 # prefer server-provided cursor, fallback to skip+limit
...
#     Notes:
#     - HubSpot doesn't have a separate 'Lead' object; we return Deals as leads.
#     - Pipedrive: will try /leads first; on 403/404, fallback to /deals.
...
#     async with httpx.AsyncClient(timeout=30.0) as client:
#         if crm == "hubspot":
#             # Treat Deals as "leads"
#             params = {
#                 "limit": limit,
#                 "properties": "dealname,amount,dealstage,createdate,hs_lead_status",
#             }
#             if cursor:
#                 params["after"] = cursor
#             r = await client.get(
...
#             )
#             if not r.is_success:
#                 raise HTTPException(r.status_code, f"HubSpot deals fetch failed: {r.text}")
#             j = r.json()
#             for d in j.get("results", []):
#                 p = d.get("properties", {}) or {}
#                 name = p.get("dealname")
#                 items.append(_norm(d.get("id"), name, None, None, None, d))
#             next_cursor = ((j.get("paging") or {}).get("next") or {}).get("after")
#             raw_paging = j.get("paging") or {}
...
#             # Prefer Leads, fallback to Deals (some accounts don't have Leads enabled/scoped)
...
#             j = r.json()
#             data = j.get("data") or []
#             # /leads returns slightly different shape than /deals — normalize name
#             for d in data:
