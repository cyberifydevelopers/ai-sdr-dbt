...
# from helpers.ai_structurer import process_submission_to_appointment  # make sure path matches your project

# # --------- CONFIG ---------
# # Use your preferred default; UTC is safest if timezone is unknown.
# DEFAULT_TZ = ZoneInfo("UTC")

# # --------- HELPERS ---------
# async def read_any_payload(request: Request) -> dict:
#     """
#     Accept JSON, x-www-form-urlencoded, or multipart/form-data.
#     Files are ignored; all scalar fields are returned.
#     """
...
#     """
#     Accepts ISO date ('YYYY-MM-DD') or ISO datetime ('YYYY-MM-DDTHH:MM[:SS][Z/+/-offset]').
#     Returns tz-aware datetime. If only a date is given, sets 09:00 local by default.
#     """
#     if value is None:
#         return None
...
# # Deep, schema-agnostic pickers (best-effort for arbitrary payloads)
...
#     """Combine common separate 'date' and 'time' fields if present."""
...
#     booking_time: datetime  # expects ISO8601
...
#     - Auth via ?token=...
#     - Accepts JSON, x-www-form-urlencoded, multipart (files ignored)
#     - Best-effort extract + Typeform-style 'answers' extract
#     - Coerces booking_time to tz-aware datetime
#     - Enqueues AI structurer to build Appointment from raw
#     """
...
#     # 3) Extract from generic payload first
#     extracted: Dict[str, Any] = best_effort_extract(payload)

#     # 4) Also handle Typeform-like structure (form_response.answers)
...
#     # 5) If separate date/time fields exist, combine as a fallback
#     combo = combine_date_time_fields(payload)
#     if combo and not extracted.get("booking_time"):
#         extracted["booking_time"] = combo

#     # 6) Parse booking_time to tz-aware datetime (prevents asyncpg DataError)
#     booking_dt = parse_any_datetime(extracted.get("booking_time"))

#     # 7) Find-or-create submission within this tenant by email/phone
#     submission = None
...
#                 booking_time=booking_dt,  # tz-aware or None
#                 additional_details=extracted.get("additional_details"),
#                 raw_data=payload,  # store entire parsed payload
...
#     # 9) Enqueue AI structuring (non-blocking)
#     if bg is not None:
...
#     submission.booking_time = (
#         data.booking_time if data.booking_time.tzinfo
#         else data.booking_time.replace(tzinfo=DEFAULT_TZ)
#     )
#     submission.status = SubmissionStatus.BOOKED
#     await submission.save()
#     return {"success": True, "form_id": submission.id, "status": submission.status}
...
# @router.get("/form/generate-script")
# async def generate_script(
#     user: Annotated[User, Depends(get_current_user)]
# ):
#     """
#     Generate a ready-to-use HTML <script> snippet for the logged-in user.
#     Also provide a WordPress plugin code snippet for easy integration.
#     """
#     # ✅ Raw JS Script
#     script = f"""
# <script>
...
#     # ✅ WordPress Plugin PHP Code
#     wordpress_plugin = f"""<?php
# /*
