...
from scheduler.campaign_scheduler import nudge_once
from controllers.campaign_controller import _tick_campaign  
from fastapi import Query

# --- add in Schemas section ---
class AddLeadsToCampaignPayload(BaseModel):
    lead_ids: List[int]
    include_dnc: bool = False  # default: skip DNC

# --- add in Lead Endpoints section ---
...
async def add_leads_to_campaign(
    ...
    """
    Push selected leads into an existing campaign's queue.
    - Validates campaign belongs to user.
    - Validates each lead belongs to the same user (via its file).
    - Skips DNC unless include_dnc=True.
    - Deduplicates existing CampaignLeadProgress rows.
    - Nudges the campaign ticker.
    """
    if not payload.lead_ids:
        raise HTTPException(status_code=400, detail="No lead_ids provided")

    campaign = await Campaign.get_or_none(id=campaign_id, user_id=user.id)
    if not campaign:
        raise HTTPException(status_code=404, detail="Campaign not found for this user")

    # Only leads that belong to this user
    leads_q = Lead.filter(id__in=payload.lead_ids, file__user_id=user.id)
    if not payload.include_dnc:
        leads_q = leads_q.filter(dnc__in=[False, None])
    leads = await leads_q.all()
    if not leads:
        return {"success": True, "detail": "No eligible leads to add", "added": 0, "skipped_existing": 0}

    lead_ids = [l.id for l in leads]

    # Deduplicate by removing those already queued for this campaign
    existing = await CampaignLeadProgress.filter(
        campaign_id=campaign.id, lead_id__in=lead_ids
    ).values_list("lead_id", flat=True)
    existing_set = set(existing)
    to_create = [lid for lid in lead_ids if lid not in existing_set]

    objs = [CampaignLeadProgress(campaign_id=campaign.id, lead_id=lid, status="pending") for lid in to_create]
    if objs:
        await CampaignLeadProgress.bulk_create(objs)

    # Kick the campaign tick to start placing calls soon
    nudge_once(_tick_campaign, campaign.id, delay_seconds=1)

    return {
        "success": True,
        "detail": f"Queued {len(to_create)} new lead(s) for campaign {campaign.id}.",
        "added": len(to_create),
...
# --- add in Schemas section ---
class AddFilesToCampaignPayload(BaseModel):
    file_ids: List[int]
    include_dnc: bool = False      # default: skip DNC
    origins: Optional[List[str]] = None  # optional filter: ["CSV", "HubSpot CRM", "GHL CRM", "Monday CRM"]

# --- add in Lead Endpoints section ---
@router.post("/campaigns/{campaign_id}/add-files")
async def add_files_to_campaign(
    campaign_id: int,
    payload: AddFilesToCampaignPayload,
    user: Annotated[User, Depends(get_current_user)],
):
    """
    Push ALL leads from the selected files into an existing campaign's queue.
    - Validates campaign & files belong to user.
    ...
    # Ensure files belong to this user
    files = await FileModel.filter(id__in=payload.file_ids, user_id=user.id).all()
    if not files:
        raise HTTPException(status_code=404, detail="No matching files found for this user")

    file_ids = [f.id for f in files]

    leads_q = Lead.filter(file_id__in=file_ids)
    if not payload.include_dnc:
        leads_q = leads_q.filter(dnc__in=[False, None])
    if payload.origins:
        leads_q = leads_q.filter(origin__in=payload.origins)

    leads = await leads_q.all()
    if not leads:
        return {"success": True, "detail": "No eligible leads found in the selected files", "added": 0}

    lead_ids = [l.id for l in leads]

    # Deduplicate vs existing progress
    existing = await CampaignLeadProgress.filter(
        campaign_id=campaign.id, lead_id__in=lead_ids
    ).values_list("lead_id", flat=True)
    existing_set = set(existing)
    ...
    return {
        "success": True,
        "detail": f"Queued {len(to_create)} lead(s) from {len(file_ids)} file(s) into campaign {campaign.id}.",
        "files_considered": len(file_ids),
        "added": len(to_create),
        "already_present": len(existing_set),
        "total_in_files": len(lead_ids),
    }
