...
class MessageScheduledRequest(BaseModel):
    ...
    # selection/backoff knobs
    include_unowned: Optional[bool] = Field(default=True, description="Include BOOKED appts with null user_id")
...
class DaemonConfigRequest(BaseModel):
    ...
    include_unowned: Optional[bool] = None  # scheduled only (BOOKED)
    repeat_backoff_hours: Optional[int] = Field(default=None, ge=0, le=72)  # scheduled (BOOKED)
...
# ─────────────────────────────────────────────────────────────────────────────
# Backoff logic (BOOKED selection gate)
# ─────────────────────────────────────────────────────────────────────────────
...
async def _eligible_booked_appointments(
    *,
    db_user: User,
    ...
    if include_unowned:
        appts_all = await Appointment.filter(status=AppointmentOutcome.BOOKED).filter(
            Q(user=db_user) | Q(user_id=None)
        ).all()
    else:
        appts_all = await Appointment.filter(user=db_user, status=AppointmentOutcome.BOOKED).all()
...
# ─────────────────────────────────────────────────────────────────────────────
# SCHEDULED (BOOKED) / UNSCHEDULED (FOLLOW_UP_NEEDED)
# ─────────────────────────────────────────────────────────────────────────────
...
async def message_scheduled_appointments(
    ...
    """
    "scheduled" now maps to BOOKED appointments.
    """
    try:
        db_user = await User.get(id=user.id)
        if payload.assistant_id:
        ...
        appts = await _eligible_booked_appointments(
            db_user=db_user,
        ...
        if not appts:
            total_all = await Appointment.filter(status=AppointmentOutcome.BOOKED).count()
            total_user = await Appointment.filter(user=db_user, status=AppointmentOutcome.BOOKED).count()
            return {
                "success": True,
                "sent": 0,
                "results": [],
                ...
                "stats": {
                    "total_booked_all": total_all,
                    "total_booked_for_user": total_user,
                    "include_unowned": bool(payload.include_unowned),
        ...
        raise HTTPException(status_code=400, detail=f"Error messaging booked appointments: {e}")
...
async def message_unscheduled_appointments(
    ...
    """
    "unscheduled" now maps to FOLLOW_UP_NEEDED appointments.
    """
    try:
        db_user = await User.get(id=user.id)
        if payload.assistant_id:
        ...
        # All FOLLOW_UP_NEEDED for this user
        appts_all = await Appointment.filter(user=db_user, status=AppointmentOutcome.FOLLOW_UP_NEEDED).all()
        ...
        raise HTTPException(status_code=400, detail=f"Error messaging follow-up-needed appointments: {e}")
...
async def debug_scheduled_selection(
    ...
    """
    Debug BOOKED selection (previously 'scheduled').
    """
    db_user = await User.get(id=user.id)
    ...
    info = {
        ...
        "repeat_backoff_hours": backoff_h,
        "rule": "BOOKED = eligible unless already messaged successfully within backoff window.",
    }
    logger.info(f"[debug] booked_selection: {info}")
    return {"info": info, "samples": samples[:50]}

# ─────────────────────────────────────────────────────────────────────────────
# Always-on Texting Daemons (scheduled=BOOKED & unscheduled=FOLLOW_UP_NEEDED)
# ─────────────────────────────────────────────────────────────────────────────
...
async def _users_with(kind: Kind) -> List[int]:
    if kind == "scheduled":
        ids = await Appointment.filter(status=AppointmentOutcome.BOOKED).values_list("user_id", flat=True)
    else:
        ids = await Appointment.filter(status=AppointmentOutcome.FOLLOW_UP_NEEDED).values_list("user_id", flat=True)
