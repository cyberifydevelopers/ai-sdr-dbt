...
async def scan_once(limit: int = BATCH_LIMIT) -> Dict[str, Any]:
    logger.info("[Scanner] Running one scan… (limit=%s)", limit)
    ...
    for sub in todo:
        try:
            logger.info("[Scanner] Processing submission_id=%s", sub.id)
            res = await process_submission_to_appointment(sub.id)
        except Exception as e:
            logger.exception("[Scanner] Unexpected error for submission_id=%s", sub.id)
            res = {"ok": False, "reason": "unexpected_error", "detail": str(e), "submission_id": sub.id}
        results.append({"submission_id": sub.id, **res})

    logger.info("[Scanner] Done. Processed=%s/%s", len(todo), len(candidates))
    return {"processed_count": len(todo), "results": results}


async def scheduler_loop(app) -> None:
    app.state._intake_scheduler_running = True
    logger.info("[Scheduler] Started intake worker loop")
    try:
        while app.state._intake_scheduler_running:
            try:
                await scan_once(BATCH_LIMIT)
            except Exception:
                logger.exception("[Scheduler] Error in scan_once")
            await asyncio.sleep(SCAN_INTERVAL_SECONDS)
    finally:
        app.state._intake_scheduler_running = False
        logger.info("[Scheduler] Stopped intake worker loop")
